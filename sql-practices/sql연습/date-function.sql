...
-- YYMM, YYYYMM
-- 예) 근무개월 수
select period_diff(date_format(curdate(), '%Y%m'), date_format(hire_date, '%Y%m')) as 'month' from employees order by month desc;
...
-- signed(unsigned), int(integer), medium int, big int, int(기본 11) -> 10000000000까지 표현 가능
-- float, double
-- time, date, datetime
