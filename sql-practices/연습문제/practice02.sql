...
-- 예) 2014년 07월 10일
select date_format(hire_date, '%Y년 %m월 %d일') as '신입사원이 들어온 날' from employees order by hire_date desc limit 0, 1;
select date_format(max(hire_date), '%Y년 %m월 %d일') as '신입사원이 들어온 날' from employees;
...
select date_format(now(), '%Y') - date_format(max(birth_date), '%Y'), date_format(now(), '%Y') - date_format(min(birth_date), '%Y') from employees;